...
mod linux_tray {
    ...
    impl ksni::Tray for MutuiTray {
        ...
        fn icon_pixmap(&self) -> Vec<ksni::Icon> {
            // 22x22 ARGB32 (network byte order) cyan circle keeps icon visible
            // even when the icon theme does not provide audio-x-generic.
            let size: i32 = 22;
            ...
            for y in 0..size {
                for x in 0..size {
                    ...
                    if dx * dx + dy * dy <= r * r {
                        data.extend_from_slice(&[255, 0, 190, 220]);
                    } else {
...
fn main() {
    eprintln!("mutui-tray is currently supported only on Linux.");
}
